...
const $ = new Env('Chiphell')
const CookieKey = "ChiphellCookie";
...
async function checkin() {
  const myRequest = {
    url: `https://www.chiphell.com/home.php?mod=space&uid=431576&do=profile&mobile=2`,
    headers: {
      'Cookie' : cookie,
  ...
  return new Promise((resolve) => {
    $.get(myRequest, (error, response, data) => {
      if (response.statusCode == 200) {
        const point = data.match(/积分.*<span>(\d+)</)[1];
        
        $.get(myRequest, (error, response, data) => {
          if (response.statusCode == 200) {
            const newPoint = data.match(/积分.*<span>(\d+)</)[1];
            if (newPoint - point > 0) {
              $.msgBody = `签到成功！🎉 积分：${newPoint}`;
            } else {
              $.msgBody = `重复签到！🎉 积分：${newPoint}`;
            }
          } else {
            $.msgBody = `检查积分失败 ‼️\n${data}`;
          }
