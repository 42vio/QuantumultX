...
const body =  'formhash=43e0c65a&signsubmit=yes&handlekey=signin&emotid=1&referer=https%3A%2F%2Fwww.tekqart.com%2F&content=%E8%AE%B0%E4%B8%8A%E4%B8%80%E7%AC%94%EF%BC%8Chold%E4%BD%8F%E6%88%91%E7%9A%84%E5%BF%AB%E4%B9%90%EF%BC%81'; 
  ...
  return new Promise((resolve) => {
    $.post(myRequest, (error, response, data) => {
      try {
        ...
        if (error && !data) {
          $.msgBody = `请求失败!\n${error}`;
        } else if (data.search("签到成功") != -1) {
          $.msgBody = "签到成功！🎉";
        } else if (data.search("已经签过") != -1) {
          $.msgBody = "今日已签过 ⚠️";
        } else {
          $.msgBody = `签到失败 ‼️\n${data}`;
        }
